 ...
 * @version 0.00.00.01
 */
public class InventoryObjects extends java.lang.Object
{
    /** Object list. */    
    public Vector objects = null;
    
    protected InventoryObjects()
    {
    }
    
    public static InventoryObjects loadObjects(Context context, InputStream stream) throws IOException
    {
        InventoryObjects o = new InventoryObjects();
        
        o.loadObjectTable(context, stream);
        return o;
    }
     ...
     *
     * @param  context  Context object. Used to determine Amiga support. Can be
     *                  null (Which will disable Amiga support)
     ...
     */    
    protected short loadObjectTable(Context context, InputStream stream) throws IOException
    {
        ...
        if (context != null)
        {
            padSize = context.getAmiga()? (byte)4: (byte)3;
        }
        else
        {
            padSize = (byte)3;
        ...
        for (i = 0; i < nobject; i++)
        {
            obj = new InventoryObject(
                            bstream.lohiReadUnsignedShort(),
                            bstream.lohiReadUnsignedByte());
            objects.add(obj);
        ...
        for (i = 0; i < nobject; i++)
        {
            obj      = (InventoryObject)objects.get(i);
            obj.name = (String)hash.get(new Integer(obj.offset));
        }
        
        bstream.close();
        return (short)nobject;
    }
     ...
     *         the Hash key.
     */    
    protected Hashtable loadStringTable(InputStream stream, int offset) throws IOException
    {
        Hashtable h = new Hashtable(objects.size() + 10);
        String    o = new String();
        int       s = offset;
        int       c;
    ...
    */    
    ...
    {
        return (InventoryObject)objects.get(index);
    }
    
    /**
     * Resets all object contained in the Object Table to their original
     * location.
     */
    public void reset()
    {
        ...
        {
            inv = (InventoryObject)enum.nextElement();
            inv.reset();
        }
    }
}